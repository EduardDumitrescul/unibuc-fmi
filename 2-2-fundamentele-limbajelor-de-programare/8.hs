...
annotate_aux (Var s) fvars = (AVar s, fvars)
annotate_aux (Lam x t) (h:tail) = (ALam x h annlambda, list)
    where (annlambda, list) = annotate_aux t tail
annotate_aux (App t1 t2) fvars = 
    let 
        (at1, laux) = annotate_aux t1 fvars
        (at2, list) = annotate_aux t2 laux
    in (AApp at1 at2, list) 
