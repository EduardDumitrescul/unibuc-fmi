...
class B
{
 ...
 { //(B &)(*this)=od ;
 /* trebuie conversie cu rezultat intors prin referinta - daca se face conversie spre B se va intoarce temporarul
 (o copie  a lui (*this) )
 -apel explicit al operatorului = din baza (prin conversia catre B &)
 -alfel se apeleaza infinit  operatorul = din D */
B &rb=(*this); rb=od; // acelasi lucru fara conversie
 (*this).B::operator=(od); // acelasi lucru -explicit
    cout<<"= derivata\n";
}
